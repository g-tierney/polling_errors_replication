...
model{
  ...
  mu_alpha ~ normal(0, 0.05);
  sigma_alpha ~ normal(0, 0.2);
  sigma_tau ~ normal(0, 0.05);
  sigma_gamma ~ normal(0,0.01); //~\pm 2pp, (.5 + rnorm(10000,0,abs(rnorm(100,0,0.01)))) %>% quantile(probs = c(.025,.975))
  ...
  for(i in 1:N_poll){
    p[i] = fmin(1,fmax(0,z[r[i],t_int[i]]));
